...
export const users = pgTable('users', {
  ...
  // System admin flag - grants global administrative privileges
...
/**
 * Chat AI Context table - stores conversation summaries for efficient context loading
 * Updated periodically (not every message) to save AI tokens
 * Used when AI resumes to understand conversation state
 */
export const chatAiContext = pgTable(
  'chat_ai_context',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    // Customer sentiment from last interaction
    customerSentiment: varchar('customer_sentiment', { length: 50 }),
    // Open questions or pending issues
    pendingItems: jsonb('pending_items').default([]),
    // Last topic discussed
    lastTopic: varchar('last_topic', { length: 255 }),
  ...
  (table) => ({
    chatIdIndex: index('idx_chat_ai_context_chat_id').on(table.chatId),
    lastSummarizedIndex: index('idx_chat_ai_context_last_summarized').on(
      table.lastSummarizedAt,
    ),
  }),
);

export type ChatAiContext = typeof chatAiContext.$inferSelect;
export type NewChatAiContext = typeof chatAiContext.$inferInsert;
...
/**
 * System AI Goal Prompts table - customizable goal prompts that apply globally
 * Only modifiable by system admins (users.is_system_admin = true)
 */
export const systemAiGoalPrompts = pgTable(
  'system_ai_goal_prompts',
  {
    id: serial('id').primaryKey(),
    goalType: varchar('goal_type', { length: 50 }).notNull().unique(), // 'answer_faq', 'qualify_lead', etc.
    displayName: varchar('display_name', { length: 100 }).notNull(),
    description: text('description'),
    promptTemplate: text('prompt_template').notNull(), // The actual prompt text
    isActive: boolean('is_active').default(true),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
    createdBy: integer('created_by').references(() => users.id),
  ...
  (table) => ({
    goalTypeIndex: index('idx_system_ai_goal_prompts_goal_type').on(
      table.goalType,
    ),
    isActiveIndex: index('idx_system_ai_goal_prompts_is_active').on(
      table.isActive,
    ),
  }),
);
...
/**
 * System AI Settings table - global AI settings (e.g., data collection instructions)
 * Only modifiable by system admins
 */
export const systemAiSettings = pgTable(
  'system_ai_settings',
  {
    id: serial('id').primaryKey(),
  ...
  (table) => ({
    settingKeyIndex: index('idx_system_ai_settings_setting_key').on(
      table.settingKey,
    ),
  }),
);
...
// Relations for system AI tables
export const systemAiGoalPromptsRelations = relations(
  systemAiGoalPrompts,
  ({ one }) => ({
    creator: one(users, {
      fields: [systemAiGoalPrompts.createdBy],
      references: [users.id],
    }),
    updater: one(users, {
      fields: [systemAiGoalPrompts.updatedBy],
      references: [users.id],
    }),
  }),
);

export const systemAiSettingsRelations = relations(
  systemAiSettings,
  ({ one }) => ({
    updater: one(users, {
      fields: [systemAiSettings.updatedBy],
      references: [users.id],
    }),
  }),
);
