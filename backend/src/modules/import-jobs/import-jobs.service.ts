...
export class ImportJobsService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (this.useLocalProcessing) {
            this.logger.log('UseLocalProcessing ENABLED: Executing Lambda code locally via child process.');
        }
    }
    ...
    private async invokeLocalHandler(payload: any): Promise<void> {
        ...
        const runnerPath = path.resolve(process.cwd(), '../infrastructure/lambda/contacts-import/src/local-runner.ts');
    ...
    private mapJobToResponse(job: ImportJob): ImportJobResponse {
        return {
            ...
            userId: job.userId,
            originalFilename: job.originalFilename,
            status: job.status as "UPLOADED" | "MAPPED" | "VALIDATED" | "QUEUED" | "PROCESSING" | "IMPORTED" | "FAILED",
