...
import { backendApi, UpdateContactDto } from "@/lib/api/endpoints";
...
export default function EditContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await backendApi.contacts.update(contactId, payload);
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <Button onClick={() => router.push(`/${locale}/dashboard/contacts`)}>
            Back to Contacts
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("updateContact")}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button type="submit" disabled={isLoading}>
                {isLoading ? `${t("updateContact")}...` : t("updateContact")}
              </Button>
            </div>
          </form>
        </CardContent>
