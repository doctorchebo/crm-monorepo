/**
 * WhatsApp Cloud API Media Limits (Frontend)
 *
...
export function validateWhatsAppMedia(
    ...
    if (mediaCategory === "video") {
      errors.push(
        `Video${fileNamePart} is too large. ` +
          `Size: ${formatBytes(fileSize)}, Maximum: ${formatBytes(
            uploadLimit
          )}. ` +
          `Videos up to ${formatBytes(
            uploadLimit
          )} can be uploaded and will be automatically compressed for WhatsApp.`
      );
    } else {
      errors.push(
        `File${fileNamePart} is too large for WhatsApp. ` +
          `Size: ${formatBytes(fileSize)}, Maximum for ${
            mediaCategory || "this type"
  ...
  if (needsCompression) {
    warnings.push(
      `Video (${formatBytes(fileSize)}) exceeds WhatsApp's ${formatBytes(
        sendLimit
      )} limit. ` + `It will be automatically compressed after upload.`
    );
  }
  ...
  if (
    ...
    warnings.push(
      `File size (${formatBytes(
        fileSize
      )}) is close to WhatsApp's ${formatBytes(
        sendLimit
      )} limit for ${mediaCategory}s. ` +
  ...
  if (mediaCategory === "video" && mimeType !== "video/mp4") {
    warnings.push(
      `WhatsApp works best with MP4 (H.264) videos. The compression process will convert to MP4 if needed.`
    );
  }
 ...
 * Useful for setting form validation limits.
 *
