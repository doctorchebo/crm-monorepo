...
export function RoleManager({ teamId }: RoleManagerProps) {
  ...
  } = useSWR<Role[]>(["team-roles", teamId], () => getTeamRoles(teamId), {
    revalidateOnFocus: false,
  });

  const { data: allPermissions, isLoading: permsLoading } = useSWR<
    Permission[]
  >(["system-permissions"], () => getSystemPermissions(), {
    revalidateOnFocus: false,
  });

  // --- Search, Pagination & Selection ---

  const roleSearchFn = useCallback(
  ...
  const getRoleId = useCallback((role: Role) => String(role.id), []);
  ...
  const {
    ...
    toggleSelectAll,
    clearSelection,
  } = useClientFilteredData<Role>({
    data: roles,
    searchFn: roleSearchFn,
    getItemId: getRoleId,
    initialPageSize: 12,
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        deletableIds.map((id) => deleteTeamRole(teamId, Number(id))),
      );
      ...
      if (succeeded > 0) {
        addNotification(
          t("notifications.bulkDeleted", { count: succeeded }),
          "success",
        );
      }
      if (failed > 0) {
        addNotification(
          t("notifications.bulkDeleteFailed", { count: failed }),
          "error",
        );
      }
      ...
      addNotification(
        t("notifications.bulkDeleteFailed", { count: selectedCount }),
        "error",
      );
    } finally {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedRoles.map((role) => (
              <RoleCard
                key={role.id}
                role={role}
                isSelected={selectedIds.has(String(role.id))}
...
function RoleCard({
  ...
  return (
    <Card
      className={`flex flex-col h-full transition-colors ${isSelected ? "ring-2 ring-primary" : ""}`}
    >
...
function RoleEditorDialog({
  ...
  const [selectedPermissions, setSelectedPermissions] =
    useState<Set<number>>(initialPerms);
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={
                    isOwner || (role?.isSystem && role?.name === "Admin")
                  }
                    ...
                    <div key={category} className="space-y-3">
                      <div className="flex items-center justify-between bg-muted/30 p-2 rounded-md">
                        <div className="flex items-center gap-2">
                          <Label className="font-semibold capitalize text-sm">
                            {category}
                          </Label>
